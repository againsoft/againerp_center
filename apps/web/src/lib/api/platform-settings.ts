...
export async function testPlatformSetting(key: string): Promise<{ ok: boolean; status: string }> {
  return apiFetch<{ ok: boolean; status: string }>(`/api/v1/platform-settings/${key}/test`, {
    method: "POST",
  });
}
