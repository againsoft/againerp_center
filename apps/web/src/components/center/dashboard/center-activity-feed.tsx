...
import type { CenterDashboardActivity } from "@/lib/mock-data/center";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";

const categoryConfig: Record<
  CenterDashboardActivity["category"],
...
export function CenterActivityFeed({ activity }: Props) {
  return (
    <div className="rounded-lg border bg-card p-4">
      <div className="mb-3 flex items-center justify-between">
        <div>
          <h2 className="text-sm font-medium">Recent platform activity</h2>
          <p className="text-xs text-muted-foreground">From audit log — operator and system actions</p>
        </div>
      ...
      {activity.length === 0 ? (
        <CenterEmptyState
          ...
          description="Actions appear here when you create clients, licenses, or subscriptions."
        />
      ) : (
